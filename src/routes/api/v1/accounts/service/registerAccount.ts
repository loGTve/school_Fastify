...
import {
  validateDate, validateBloodType,
  validateEmailAddress,
  validateGender, validateMbtiType,
  validatePhoneNumber
} from "../../../../../utils/common";
...
const registerAccountService = async (
  ...
  const findSameEmail = await fastify.pg.transact<SelectAccountEmailResult>(async client => {
    ...
    return <SelectAccountEmailResult> humps.camelizeKeys((await client.query<SelectAccountEmailResult>(query)).rows)[0];
  });
  ...
  const findSameNickname = await fastify.pg.transact<SelectAccountNicknameResult>(async client => {
    ...
    return <SelectAccountNicknameResult> humps.camelizeKeys((await client.query<SelectAccountNicknameResult>(query)).rows)[0];
  });
  ...
  const insertAccount = await fastify.pg.transact<InsertAccountResult>(async client => {
    ...
    return <InsertAccountResult> humps.camelizeKeys((await client.query<InsertAccountResult>(query)).rows)[0];
  });

  await fastify.pg.transact(async client => {
    const query: QueryType = {
      text: `INSERT INTO app_settings (account_id,
                                       visibility_mbti,
